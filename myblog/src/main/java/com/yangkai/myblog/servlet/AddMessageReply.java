...
public class AddMessageReply extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
			...
			String guest=null;
			String message_reply_content=EncoderUtil.encode(request.getParameter("message_reply_content"));
			...
			if(message_link_mark==Constants.ADD_MESSAGE_REPLY_FROM_USER_MESSAGE  || 
							message_link_mark==Constants.ADD_MESSAGE_REPLY_FROM_USER_MESSAGE_VIEW || 
							message_link_mark==Constants.ADD_MESSAGE_REPLY_FROM_USER_MESSAGE_FROM_FRIENDS_DYNAMIC){
			...
			if(count>0)
			{
				...
				if(message_link_mark==Constants.ADD_MESSAGE_REPLY_FROM_USER_MESSAGE_VIEW)
				{
					response.sendRedirect("message_view.jsp?message_id="+message_id);
					return;
				}
				if(message_link_mark==Constants.ADD_MESSAGE_REPLY_FROM_USER_MESSAGE_FROM_FRIENDS_DYNAMIC)
				{
