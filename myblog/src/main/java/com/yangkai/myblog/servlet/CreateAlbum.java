...
public class CreateAlbum extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
			...
			if(album_name!=null)
			{
				album_name=EncoderUtil.encode(album_name);
				...
				if(!albumDir.exists())
				{
					/*下面代码应该属于业务逻辑代码，应该放在业务逻辑层。*/
					boolean isCreate=albumDir.mkdir();
					if(isCreate==true)//这样的判断多此一举。
					{
						...
						{
							album_desc=EncoderUtil.encode(album_desc);
						}
...
//							System.out.println(create_date);
							...
							response.sendRedirect("include/create_success.jsp?album_name="+URLEncoder.encode(album_name,"UTF-8"));
						}
					}else{
