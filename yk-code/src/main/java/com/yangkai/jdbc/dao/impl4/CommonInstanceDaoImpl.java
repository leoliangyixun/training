...
public class CommonInstanceDaoImpl  implements CommonDAO{
	...
	public <T> T findById(Class<T> clazz,int id) {
		...
		Object[] args={id};
		T t=template.find(clazz,sql, args,new ObjectMapper() 
		{
	...
	public <T> List<T> findAll(Class<T> clazz, int id) {
		...
		String sql="SELECT * FROM "+tableName+" WHERE id<?";
		Object[] args={id};
		List<T> list=template.findAll(clazz,sql, args,new ObjectMapper() {
			@SuppressWarnings("hiding")
			@Override
			public <T> T commonObjectMapper(ResultSet rs,Class<T> clazz) { 
				T t=null;
