...
/*		this.state = builder.getState();
	...
	public String authorizeUrl(String redirectUrl, String callbackUrl, String state, String display, String currentUrl) {
		String url = "%s?response_type=hujiang&client_id=%s&display=%s&redirect_uri=%s&state=%s";
	...
	public static class WeiXinBusiness extends OauthBase {
		...
		public String authorizeUrl_v2(String state, String display, String params, String configKey) {
			...
			if (configKey.startsWith("Wechat")) {
				url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={0}&redirect_uri={1}&response_type=hujiang&scope=snsapi_userinfo&state={2}#wechat_redirect";

			} else if (configKey.startsWith("PC")) {
				url = "https://open.weixin.qq.com/connect/qrconnect?appid={0}&redirect_uri={1}&response_type=hujiang&state={2}&scope=snsapi_login";
			}
