			...
			public void run() {
				while (true) {
					...
					if (vipCustomerNumber != null) {
						System.out.println(windowNumber + "号VIP窗口开始为"+ vipCustomerNumber 
								+ "号VIP客户服务......");	
						...
						System.out.println(windowNumber + "号VIP窗口为"
								+ vipCustomerNumber + "号VIP客户服务了" + serviceTime+ "s");
						...
						if (commonCustomerNumber != null) {
							System.out.println(windowNumber + "号VIP窗口开始为"+ commonCustomerNumber + "号普通客户服务");
							int serviceTime=new Random()
								.nextInt(Constants.MAX_SERVICE_TIME - Constants.MIN_SERVICE_TIME + 1) 
								+ Constants.MIN_SERVICE_TIME;
							try {
