package hujiang.framework.plugin.dfs;
...
public class TakaServiceTest2 {
    ...
    public void testAdvancedUpload() throws Exception {
        ...
        file.setRawFilename(resource.getFilename());
        file.setContent(IOUtils.toByteArray(resource.getInputStream()));
        file.setBucket("hj-courseware");
        //file.setCustomPath("/dsp/creative/xxx.jpg");
        UploadInfo info = takaService.advancedUpload(file);
        System.out.println(JsonUtil.object2JSON(info));
    }

    @Test
    public void testAdvancedUpload2() throws Exception {
        AdvanceFile file = new AdvanceFile();
        ...
        file.setFiles(files);
        List<UploadInfo> infos = takaService.advancedUpload2(file);
        System.out.println(JsonUtil.object2JSON(infos));
    }
