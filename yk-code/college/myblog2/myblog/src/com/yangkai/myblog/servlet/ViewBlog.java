...
//		isFriend=Boolean.parseBoolean(request.getParameter("isFriend"));
		...
		System.out.println("ViewBlog:"+service);
		List<BlogComment> blog_comment=service.getBlogComment(blog_id);//获取该博文对应的评论
		//blog_comment=(List<BlogComment>) session.getAttribute(" blog_comment");
		 ...
		 * */
		Map<Integer,List<BlogCommentReply>> blog_comment_reply_map=new HashMap<Integer,List<BlogCommentReply>>();//获取评论编号-回复信息的Map集合。
		if(blog_comment!=null)
		{
			/*
			 * 根据博文评论编号获取对应的回复信息
			...
			if(blog_comment_reply_map.size()<1)
			{
				blog_comment_reply_map=null;
			}
		}
		else{
			blog_comment_reply_map=null;
